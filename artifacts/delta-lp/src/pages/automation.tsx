    ...
    <div className="delta-rise">
      ...
      <div className="card-gradient max-w-3xl rounded-xl border p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          ...
          <button 
            onClick={() => connected && onTargetNetwork && setEnabled(!enabled)}
            aria-label="Toggle automation" 
            className={`relative h-6 w-11 rounded-full transition focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background ${!connected || !onTargetNetwork ? "opacity-50 cursor-not-allowed bg-[#294a32]" : enabled ? "bg-primary" : "bg-[#294a32]"}`}
          >
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2">
          <div className="rounded-lg border border-[#6aa4771c] bg-[#08150e] p-4">
            ...
            <div className="mt-3 flex items-center gap-2 text-sm">
              {enabled && connected && onTargetNetwork ? (
